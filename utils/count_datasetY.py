...
def count_classes(dataset_name, Y_RealLabel, show=False, pause_sec=2):
    ...
    # 左图：类别统计
    colors = np.where(is_rare, "#D55E00", "#4C72B0")
    bars = ax_count.bar(classes, counts, color=colors, edgecolor="black", linewidth=0.8)
    ...
    if is_rare.any():
        ax_count.text(0.01, 0.98, f"红色柱体 = 稀有类（Count < {rare_th:.1f}）",
    ...
    ax_rank.plot(ranks, counts_sorted, marker="o", color="#4C72B0", linewidth=1.6)
    ax_rank.set_xlabel("频次排名（Rank）")
    ax_rank.set_ylabel("样本数（Count）")
    ax_rank.set_title(f"{dataset_name} - 长尾与覆盖率分析", fontsize=14)
    ax_rank.set_ylim(0, max(counts_sorted) * 1.15)
    # ✅ 强制横坐标刻度为整数，并自动控制显示密度（避免挤成一团）
    ax_rank.xaxis.set_major_locator(MaxNLocator(integer=True, nbins=8))
    ax_cum = ax_rank.twinx()
    ax_cum.plot(ranks, cum, linestyle="--", marker=".", color="#D55E00", linewidth=1.2)
    ax_cum.set_ylabel("累计占比（%）")
    ax_cum.set_ylim(0, 105)
    # Top-K 标注：相邻标注上下交替错开，避免文字互相遮挡；末位 Top-K 恒为 100% 且贴边易截断，不标注
    top_keys = [1, 3, 5, 10, 20, 50, 100] if K >= 20 else [1, 2, 3, 5, 10]
    label_step = max(counts_sorted) * 0.05
    for idx, k in enumerate(top_keys):
        if len(cum) > k:
            offset = label_step if idx % 2 == 0 else -label_step
            y_pos = max(counts_sorted[k - 1] + offset, 0.0)
